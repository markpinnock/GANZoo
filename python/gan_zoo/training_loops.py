...
def training_loop(config, idx, Model, data, latent_sample, fade=False):
    ...
    IMG_SAVE_PATH = f"{config['SAVE_PATH']}imgs/{config['EXPT_NAME']}/"
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 1) % 1 == 0 and not fade:
            ...
            plt.savefig(f"{IMG_SAVE_PATH}epoch_{epoch + 1:02d}.png", dpi=250)
            plt.close()

        # Save checkpoint
        if (epoch + 1) % 1 == 0:
            check_path = f"{config['SAVE_PATH']}models/DCGANTest1/"
            ...
            Model.Generator.save(f"{check_path}/generator")
